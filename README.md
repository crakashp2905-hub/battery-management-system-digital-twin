...
<p>
  <img alt="version" src="https://img.shields.io/badge/version-0.56.0-blue">
...
> **Status.** ✅ 519/519 unit tests pass • 65 library modules • 7 chemistries • ruff-clean •
...
- **Validated on real data** — run against the **NASA PCoE** aging dataset (public domain) out of
...
- **Reproducible & tested** — every randomness source is seeded; 519 unit tests; pip-installable
...
│   ├── pack_twin.py           # Per-cell pack twin: limiting/weakest cell, per-cell SoH
│   ├── prognostics.py         # Probabilistic runaway P(t), RUL distribution, failure lead time
│   ├── fleet.py               # Fleet twins: population health, at-risk ranking, clusters
...
│   ├── afe.py                 # Analog front-end model (ADC quant, noise, current bandwidth)
...
│   ├── uncertainty.py         # Uncertainty calibration: coverage, ECE, reliability, sigma-scale
...
│   ├── counterfactual.py      # "What if I charged at 1C vs 3C?" — policy + degradation
│   ├── autonomy.py            # Autonomous learning loop: observe→design→excite→calibrate
...
│   ├── fta.py                 # Fault Tree Analysis (cut sets, top-event probability, mermaid)
│   ├── charging.py            # AC/DC, fast/slow charging physics + plating limit
│   ├── charge_control.py      # MPC/optimal fast charging (V/T/plating-limited)
│   ├── aging.py               # Dynamic capacity fade + resistance growth; SoH feedback to pack
│   ├── reliability.py         # Monte-Carlo fleet life + warranty curves (B10, RUL CI)
│   ├── second_life.py         # Second-life verdict, residual value, LCOE economics
│   ├── control.py             # Supervisor FSM + precharge (RC DC-link plant) + SoH-aware control
│   ├── control_core.py        # Firmware-portable core (SoC EKF + SoP + safety, C-ready) + SIL
...
│   ├── uds.py                 # UDS (ISO 14229) diagnostic server (DIDs, DTCs) + CAN FD
...
│   ├── grid_storage.py        # Stationary storage dispatch (peak-shave/arbitrage) + storage SoC
│   ├── interpret.py           # Plain-language explanations, feature importances, fusion
│   ├── report.py              # One-command self-contained HTML health/validation report
│   ├── api.py                 # FastAPI live-twin service (POST /step -> SoC + drift)
...
├── tests/test_bms.py          # 519 unit tests
...
├── docs/twin.md               # Central BatteryDigitalTwin state + confidence model
├── docs/firmware.md           # Control-core → embedded C, with the twin as SIL oracle
├── docs/safety_case.md        # FMEA + Fault Tree + traceability (every hazard → a test)
├── docs/validation.md         # Real-data validation harness + dataset guide
├── scripts/benchmark_estimators.py  # Head-to-head SoC RMSE across chemistries/conditions
├── Dockerfile                 # Container for the live API service
...
| **Uncertainty calibration — coverage / ECE / reliability / σ-recalibration** | `bms/uncertainty.py` |
...
| **Self-calibrating twin — observability-gated recalibration** | `bms/self_calibration.py` |
